...
class XSensDriver(rclpy.node.Node):
    ...
    def convert_coords(self, x, y, z, source, dest=None):
        if dest is None:
    ...
    def convert_quat(self, q, source, dest=None):
        if dest is None:
            dest = self.frame_local
    ...
    def publish_time_ref(self, secs, nsecs, source):
    ...
    def stamp_from_itow(self, itow, y=None, m=None, d=None, ns=0, week=None):
    ...
    # MTData
    def fill_from_RAW(self, raw_data):
    ...
    def fill_from_RAWGPS(self, rawgps_data):
    ...
    def fill_from_Orient(self, orient_data):
    ...
    def fill_from_Auxiliary(self, aux_data):
    ...
    def fill_from_Pos(self, position_data):
    ...
    def fill_from_Vel(self, velocity_data):
    ...
    def fill_from_Stat(self, status):
    ...
    def fill_from_Sample(self, ts):
        """Catch 'Sample' MTData blocks."""
        self.h.seq = ts

    # MTData2
    def fill_from_Temperature(self, o):
    ...
    def fill_from_Timestamp(self, o):
        ...
        try:
            ...
            if f & 0x4:
                ...
                self.publish_time_ref(secs, ns, 'UTC time')
        except KeyError:
            pass
        try:
            itow = o['TimeOfWeek']
            secs, nsecs = stamp_from_itow(itow)
            self.publish_time_ref(secs, nsecs, 'integer time of week')
        except KeyError:
            pass
        try:
            ...
            self.h.stamp.sec = secs
            self.h.stamp.nanosec = int(nsecs)
            self.publish_time_ref(secs, nsecs, 'sample time fine')
        except KeyError:
            pass
        try:
            sample_time_coarse = o['SampleTimeCoarse']
            self.publish_time_ref(sample_time_coarse, 0, 'sample time coarse')
        except KeyError:
            pass
    ...
    def fill_from_GNSS(self, o):
        ...
        try:  # PVT
            ...
            if f & 0x4:
                ...
                self.publish_time_ref(secs, nsecs, 'GNSS time UTC')
            # flags
            fixtype = o['fixtype']
            if fixtype == 0x00:
                # no fix
    ...
    def fill_from_GPS(self, o):
        ...
        try:    # SOL
            ...
            if (f & 0x0C) == 0xC:
                ...
                self.publish_time_ref(secs, nsecs, 'GPS Time')
    ...
    def spin_once(self):
        ...
        for n, o in data.items():
            try:
                getattr(self, self.find_handler_name(n))(o)
            except KeyError:
